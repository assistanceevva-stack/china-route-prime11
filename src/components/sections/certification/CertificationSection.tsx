import iconWb from "@/assets/sections/certification/iconwb222.PNG";
import iconOzon from "@/assets/sections/certification/iconozon222.PNG";
import iconMarket from "@/assets/sections/certification/iconmarket222.PNG";
import iconYandex from "@/assets/sections/certification/iconyandex222.PNG";
...
const BADGES = [
  { src: iconWb, alt: "Wildberries" },
  { src: iconOzon, alt: "Ozon" },
  { src: iconMarket, alt: "Маркетплейсы" },
  { src: iconYandex, alt: "Яндекс Маркет" },
];
...
export default function CertificationSection() {
  return (
    <section id="certification" className="pt-[0.8cm] pb-[1.2cm] px-6 md:px-8 bg-milk border-t-2 border-emerald-brand/15" aria-label="Сертификация и маркетплейсы">
          ...
          <div className="relative z-10 py-8 md:py-10 px-6 md:px-12 grid grid-cols-1 md:grid-cols-2 gap-10 md:gap-16 items-center min-h-[320px] md:min-h-[360px]">
            ...
            <div className="flex flex-col md:flex-row md:items-center gap-8 md:gap-10 min-w-0 md:pr-8">
              ...
              >
                ...
                <svg className="w-6 h-6 md:h-8 text-white/50 flex-shrink-0" viewBox="0 0 24 16" fill="none" stroke="currentColor" strokeWidth="2">
            ...
            <div className="flex flex-col md:flex-row md:items-center gap-8 md:gap-12 min-w-0 md:pl-8">
              ...
              <div
                className="flex flex-col items-center justify-between gap-0 flex-shrink-0 w-fit h-[15rem] md:h-[17rem] md:ml-auto"
                ...
                {BADGES.map(({ src, alt }, i) => (
                  <div key={i} className="flex flex-col items-center">
                    <img
                      src={src}
                      alt={alt}
                      className="w-11 h-11 md:w-11 md:h-11 object-contain rounded-xl flex-shrink-0"
                      loading="lazy"
                    />
                    {i < BADGES.length - 1 && (
                      <svg className="w-5 h-6 md:h-8 text-white/40 flex-shrink-0" viewBox="0 0 20 16" fill="none" stroke="currentColor" strokeWidth="1.5">
